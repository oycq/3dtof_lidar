...
### 需要 `pip install` 什么？能不能直接跑？
- **可以“直接在仓库里跑”**：在仓库根目录运行 `py .\livox_controller_demo.py`，会直接导入本地的 `openpylivox/`。
- **但仍然需要安装依赖**（否则会缺包）。推荐用 editable 安装一次，把依赖装齐：
...
```bash
py -m pip install numpy crcmod deprecated laspy tqdm
py .\livox_controller_demo.py
```

### 输出文件
demo 会生成点云/IMU 文件（例如 `*.bin` / `*.las` / `*_IMU.bin.csv`）。我已经新增 `.gitignore` 来 **自动忽略这些生成数据**，避免误提交。

### 备注
- 自动发现/连接失败时，通常是电脑有多个网卡导致“选错本机 IP”，可以在 demo 里改用 `sensor.auto_connect("你的电脑网卡IP")`。
