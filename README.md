...
前 62 个 bin 是有效直方图。`net.py` 里 62 通道的中间量全部留在 10bit 量程上算——距离重心、argmax 选峰 bin、SNR、crosstalk 都直接用原始 `hist`（SNR 反映的是原始光子统计噪声，crosstalk 是同帧同曝光下跟全图均值的横向比较，都不需要聚堆补偿）。`pileup_gain` 只在最后一步乘进去，一共两处：
