                    ...
                    if self.started:
                        ...
                        if timeSinceStart <= self.duration:
                            ...
                            if select.select([self.d_socket], [], [], 0)[0]:
                                ...
                                # update datatype every packet (some devices may not match the initial packet)
                                self.dataType = int.from_bytes(data_pc[9:10], byteorder='little')
        ...
        if goodData:
            ...
            if cmdMessage == "MSG (message)" and dataMessage == "General" and dataID == "0":
                ...
                if device_type == 0:
                    ...
                    typeMessage = "UNKNOWN(" + str(device_type) + ")"
            ...
            if checkMessage == "OPENPYLIVOX":
                ...
                if firmwareType >= 1 and firmwareType <= 3:
                    ...
                    if dataType == 0 or dataType == 2 or dataType == 4:
                        ...
                        # No points parsed -> avoid crashing on np.min / LAS write
                        if len(coord1s) == 0:
                            pbari.close()
                            binFile.close()
                            print("   - No valid points found (0 pts). Skipping LAS conversion for: " + filePathAndName)
                            print()
                            time.sleep(0.2)
                            return

                        #save lists of point data attributes to LAS file
                        # laspy 2.x API (works with modern Python / numpy)
                        hdr = laspy.LasHeader(point_format=3, version="1.2")
                        hdr.system_identifier = "OpenPyLivox"
                        hdr.generating_software = "OpenPyLivox V1.1.0"

                        coord1s = np.asarray(coord1s, dtype=np.float64)
                        coord2s = np.asarray(coord2s, dtype=np.float64)
                        coord3s = np.asarray(coord3s, dtype=np.float64)

                        xmin = float(np.floor(np.min(coord1s)))
                        ymin = float(np.floor(np.min(coord2s)))
                        zmin = float(np.floor(np.min(coord3s)))
                        hdr.offsets = np.array([xmin, ymin, zmin], dtype=np.float64)
                        ...
                        las.y = coord2s
                        las.z = coord3s
                        las.gps_time = np.asarray(times, dtype=np.float64)
                        las.intensity = np.asarray(intensity, dtype=np.uint16)
                        las.return_number = np.asarray(returnNums, dtype=np.uint8)
                        las.write(filePathAndName + ".las")
