...
LOCAL_CACHE_DIR = Path(__file__).resolve().parent / "tmp"
LOCAL_OUTPUT_PATH = LOCAL_CACHE_DIR / "tof.output"
RECORD_DIR = Path(__file__).resolve().parent.parent / "r" / "tmp"

EXPECTED_BYTES = OUT_C * PIXELS * 4
READ_RETRY = 3
ADB_READ_TIMEOUT_S = 1.2
EPS = 1e-6
DISP_GAMMA = 1.2
SNR_SHOW_MAX = 10.0
...
def main() -> int:
    ...
    print(f"[adb]  pull to {LOCAL_CACHE_DIR}")
    ...
    wait_ms = max(int(round(1000.0 / max(float(FPS), 0.1))), 1)
    next_read_time = 0.0
    ...
    while True:
        ...
        if now >= next_read_time:
            try:
                ...
                last_err = str(e)
            next_read_time = now + 1.0 / max(float(FPS), 0.1)
        ...
        dist_big = _with_text(dist_big, "DIST (conf>0.5)")
        conf_big = _with_text(conf_big, "CONF")
        reflect_big = _with_text(reflect_big, "REFLECT")
        snr_big = _with_text(snr_big, "SNR")

        hover1 = f"idx={frame_idx}  hover=({px},{py})  dist={float(dist[py, px]):.4f}m  conf={float(conf[py, px]):.4f}"
        ...
        if key == 32:  # Space: toggle recording
            if rec_writer is None:
                ...
                try:
                    ...
                    writer = cv2.VideoWriter(rec_path, fourcc, max(float(FPS), 1.0), (view.shape[1], view.shape[0]))
